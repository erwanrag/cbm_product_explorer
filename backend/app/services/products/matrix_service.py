...
async def get_product_matrix_from_identifier(
    ...
    if is_payload_empty(payload):
        return ProductMatrixResponse(matrix=[])
    """
    Renvoie le groupe, la liste cod_pro, les ref_crn et ref_ext d'un produit/groupe.
    Toujours retourne un ProductMatrixResponse (Pydantic).
    """
    ...
    # 1️⃣ Résolution de la vraie cod_pro_list
    ...
    # 3️⃣ Nettoyage et sets pour éviter les doublons
    cod_pro_set = sorted(set(row[1] for row in rows if row[1]))
    ...
    # 4️⃣ On renvoie un seul groupe_crn si unique
