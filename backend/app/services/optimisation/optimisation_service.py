...
async def evaluate_group_optimization(
    ...
    """
    Évalue les gains potentiels immédiats et à 6 mois par groupe et qualité.
    ...
    try:
        ...
        # ✅ 3. Requête SQL agrégée sécurisée (Achat & Sales sur 12 derniers mois)
        query = f"""
            WITH Achat AS (
            ...
            FROM (SELECT DISTINCT grouping_crn, qualite, cod_pro 
        ...
        # ✅ 4. Récupération historique pour tendance (6 derniers mois)
        history = await _get_sales_history_for_trend(cod_pro_list, db)

        # ✅ 5. Groupement par grouping_crn + qualité
        groups = {}
        ...
        logger.debug(f"✅ Optimisation calculée: {len(items)} groupes analysés")
...
# ✅ Historique ventes (6 derniers mois) - Version sécurisée
