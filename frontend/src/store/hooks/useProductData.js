...
export function AppStateProvider({ children }) {
    ...
    // Actions encapsulées
    const actions = {
        setSidebarState: useCallback((open, pinned) =>
            dispatch({
        ...
        setFilterMode: useCallback((mode) =>
            dispatch({ type: actionTypes.SET_FILTER_MODE, payload: mode }), []),

        setCurrentProduct: useCallback((product) =>
            dispatch({ type: actionTypes.SET_CURRENT_PRODUCT, payload: product }), []),

        setSelectedProducts: useCallback((products) =>
            dispatch({ type: actionTypes.SET_SELECTED_PRODUCTS, payload: products }), []),
