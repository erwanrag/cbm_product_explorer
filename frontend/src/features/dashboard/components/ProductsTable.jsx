  ...
  const columns = useMemo(
    () => [
        ...
        renderCell: ({ value }) =>
          value ? (
            <Chip
              label={value}
              size="small"
              sx={{ bgcolor: '#fce4ec', color: '#880e4f', fontWeight: 600 }}
            />
          ) : (
            '-'
          ),
      ...
      },
      {
        ...
        renderCell: ({ value }) => (
          <Box
            sx={{ color: value <= 0 ? '#c62828' : 'inherit', fontWeight: value <= 0 ? 600 : 400 }}
          >
            {formatPrix(value)}
          </Box>
        ),
  ...
  return (
    <Box mt={4}>
      ...
      <DataGrid
        ...
        onRowClick={(params) => {
          const selected =
            String(selectedCodPro) === String(params.row.cod_pro) ? null : params.row.cod_pro;
