...
export default function ColumnPicker({ allColumns, visibility, setVisibility, onReset }) {
  const [anchorEl, setAnchorEl] = React.useState(null);

  const toggle = (field) => setVisibility((v) => ({ ...v, [field]: !v[field] }));
    ...
    <>
      ...
      <Popover open={!!anchorEl} anchorEl={anchorEl} onClose={() => setAnchorEl(null)}>
        <FormGroup sx={{ p: 2, minWidth: 200 }}>
          ...
          <Button size="small" onClick={onReset} sx={{ mt: 1 }}>
            Réinitialiser
          </Button>
        </FormGroup>
      </Popover>
